 ...
 * result titles, URLs, and snippets that the model can then read with
 ...
 * This is the discovery step the agent was previously missing (it could only
 * fetch URLs it already knew).
 */
@Singleton
class WebSearchCapability @Inject constructor(
    ...
    override suspend fun execute(input: Map<String, String>): MiasResult<String> {
        ...
        return runCatchingMias {
            ...
            val response = httpClient.get(url) {
                // DuckDuckGo serves the HTML results page to browser-like agents.
            ...
            val results = parseResults(response.bodyAsText(), limit)
            if (results.isEmpty()) {
    ...
    private fun parseResults(html: String, limit: Int): List<Result> {
