...
class MediaStoreFileGenerationCapability @Inject constructor(
    ...
    override suspend fun execute(input: Map<String, String>): MiasResult<String> = withContext(Dispatchers.IO) {
        val filename = input["filename"]
            ?: return@withContext MiasResult.Error("filename parameter is required")
        val content = input["content"]
            ?: return@withContext MiasResult.Error("content parameter is required")
        ...
        try {
            ...
            val contentValues = ContentValues().apply {
                ...
                put(MediaStore.MediaColumns.RELATIVE_PATH, Environment.DIRECTORY_DOCUMENTS + "/MiasExports")
            }

            val uri = resolver.insert(MediaStore.Files.getContentUri("external"), contentValues)
                ?: return@withContext MiasResult.Error("Failed to create target file URI in MediaStore.")

            resolver.openOutputStream(uri)?.use { outputStream ->
                outputStream.write(content.toByteArray(Charsets.UTF_8))
            }
            
            MiasResult.Success("Successfully generated $filename in Documents/MiasExports. It is immediately visible to the user.")
        } catch (e: Exception) {
            MiasResult.Error("Failed to generate file: ${e.message}", e)
        }
    }
