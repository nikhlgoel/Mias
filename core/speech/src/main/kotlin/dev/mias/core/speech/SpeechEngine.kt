...
class SpeechEngine @Inject constructor(
     ...
     */
    suspend fun startListening(): MiasResult<Unit> = withContext(Dispatchers.Default) {
        try {
            if (context.checkSelfPermission(android.Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
                ...
                return@withContext MiasResult.Error("Microphone permission not granted")
            }

            if (!SpeechRecognizer.isRecognitionAvailable(context)) {
                _state.value = SpeechState.ERROR
                return@withContext MiasResult.Error("Speech recognition is not available on this device")
            }
            ...
            MiasResult.Error(e.message ?: "Unknown speech error", e)
        }
    }
     ...
     */
    suspend fun stopListening(): MiasResult<SpeechRecognitionResult> = withContext(Dispatchers.Default) {
        try {
            _state.value = SpeechState.PROCESSING
            ...
            _state.value = SpeechState.SUCCESS
            MiasResult.Success(result)
        } catch (e: Exception) {
            _state.value = SpeechState.ERROR
            _error.value = e.message
            MiasResult.Error(e.message ?: "Recognition failed", e)
        }
    }
