...
class ManualAccessConsentGate @Inject constructor() {
    ...
    fun consumeConsent(
        ...
        val approval = approvals.remove(token)
            ?: return MiasResult.Error("Manual approval missing or already used")

        if (approval.operation != operation) {
            return MiasResult.Error("Approval operation mismatch: expected ${approval.operation}, got $operation")
        }

        if (System.currentTimeMillis() > approval.expiresAtMs) {
            return MiasResult.Error("Manual approval expired")
        }

        return MiasResult.Success(Unit)
    }
